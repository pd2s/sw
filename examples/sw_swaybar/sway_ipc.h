...
static bool32_t sway_ipc_get_socket_path(char out[static PATH_MAX]);
static int sway_ipc_connect(char socket_path[static PATH_MAX]);

static int sway_ipc_send(int fd, enum sway_ipc_message_type, string_t *payload);
static struct sway_ipc_response *sway_ipc_receive(int fd, allocator_t *alloc);

static void sway_ipc_response_free(struct sway_ipc_response *, allocator_t *alloc);
...
static int sway_ipc_connect(char socket_path[static PATH_MAX]) {
	...
	size_t len = MIN((sizeof(addr.sun_path) - 1), strlen(socket_path));
...
static void sway_ipc_response_free(struct sway_ipc_response *response, allocator_t *alloc) {
...
static struct sway_ipc_response *sway_ipc_receive(int fd, allocator_t *alloc) {
	...
	struct sway_ipc_response *response = alloc->alloc(alloc, sizeof(*response), ALIGNOF(*response));
	uint32_t len;
	...
	response->payload = (string_t){
		.s = alloc->alloc(alloc, len + 1, ALIGNOF(*response->payload.s)),
		.len = len,
		.free_contents = TRUE,
		.nul_terminated = TRUE,
	};
