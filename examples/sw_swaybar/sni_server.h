...
static void sni__item_read_pixmap( sd_bus_message *msg,
    ...
    while ((sd_bus_message_enter_container(msg, 'r', "iiay") == 1) && (*dest_count < dest_capacity)) {
        ...
        if (((size_t)width * (size_t)height * 4) == nbytes) {
            ...
            bswap32_argb32_premultiply_alpha(pixmap->pixels, (uint32_t *)(uintptr_t)bytes, ((size_t)width * (size_t)height));
#else
            argb32_premultiply_alpha(pixmap->pixels, (uint32_t *)(uintptr_t)bytes, ((size_t)width * (size_t)height));
#endif
