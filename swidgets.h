...
static pixman_color_t sw__color_argb32_to_pixman_color(sw_color_argb32_t color) {
    ...
    c.red = (uint16_t)(((((uint32_t)color.c.r * (uint32_t)color.c.a + 128) * 257) >> 16) * 257);
    c.green = (uint16_t)(((((uint32_t)color.c.g * (uint32_t)color.c.a + 128) * 257) >> 16) * 257);
    c.blue = (uint16_t)(((((uint32_t)color.c.b * (uint32_t)color.c.a + 128) * 257) >> 16) * 257);
...
static pixman_image_t *sw__render_svg(const su_allocator_t *gp_alloc, resvg_render_tree *tree,
    ...
    su_abgr32_convert_argb32(image_data->pixels, image_data->pixels, (size_t)width * (size_t)height);
...
static pixman_image_t *sw__load_png(const su_allocator_t *gp_alloc, const su_allocator_t *scratch_alloc,
    ...
    if (stbi__png_test(&ctx)) {
        ...
        if (src && (width > 0) && (height > 0)) {
            ...
            image = sw__image_create(gp_alloc, width, height, &image_data);
            su_abgr32_convert_argb32_premultiply_alpha(image_data->pixels, src, (size_t)width * (size_t)height);
        }
    ...
    if ((src = (uint32_t *)stbi__load_gif_main(&ctx, &frame_delays, &width, &height, &frame_count, &unused, 4))
        ...
        if (frame_count > 1) {
            ...
            for ( ; i < frame_count; ++i) {
                ...
                su_abgr32_convert_argb32_premultiply_alpha(frame_data->pixels,
...
static su_bool32_t sw__layout_block_prepare(sw_layout_block_t *block, sw_layout_block_dimensions_t *overrides) {
    ...
    if (overrides) {
        *dim = *overrides;
        ...
        if (block_priv->content_image &&
                block->in.content_transform && ((block->in.content_transform % 2) == 0)) {
            SU_SWAP(content_width, content_height);
        }
...
static void sw__layout_block_render(sw_layout_block_t *block, pixman_image_t *dest) {
    ...
    if (block_priv->content_image) {
        ...
        if (block->in.content_transform &&
                ((block->in.content_transform % 2) == 0)) {
            SU_SWAP(content_image_width, content_image_height);
        }
